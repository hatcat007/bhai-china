export type JewelryCase = {
  ...
  keyCollectionsEn: string[];
  materials: string[];
  materialsEn: string[];
  // ── 可信度字段（Phase 2）：Buster 的真实角色 / 运行机制 / 摩擦点。可选，渲染层条件处理。
  //    20-d 补 EN 对应字段（myRole/mechanics/friction 原始字段为中文文案）。
  myRole?: string;
  myRoleEn?: string;
  mechanics?: string;
  mechanicsEn?: string;
  friction?: { problem: string; fix: string };
  frictionEn?: { problem: string; fix: string };
  accentColor?: string;
};
...
export const jewelryCases: JewelryCase[] = [
  {
    ...
    foundedEn: "1982",
    hq: "哥本哈根 · Havneholmen",
    hqEn: "Copenhagen · Havneholmen",
    segment: "全球最大珠宝品牌 · 大众奢侈 · 串饰",
    segmentEn: "World's largest jewelry brand · mass luxury · charms",
    revenue: "DKK 317 亿（FY2024）",
    revenueEn: "DKK 31.7B (FY2024)",
    stores: "100+ 国家 · ~6,700 个销售点",
    storesEn: "100+ countries · ~6,700 points of sale",
    ...
    results: [
      { label: "China concept-store revenue per sqm", labelZh: "中国概念店每平米营收", before: "持续下滑", after: "+38% 同店增长（试点 3 店）", beforeEn: "Sustained decline", afterEn: "+38% same-store growth (3 pilot stores)" },
      { label: "WeChat VIP reactivation rate", labelZh: "微信 VIP 重新激活率", before: "2.1%（群发短信）", after: "23.4%（个性化 AI 文案）", beforeEn: "2.1% (mass SMS)", afterEn: "23.4% (personalized AI copy)" },
      { label: "Counterfeit takedown success", labelZh: "假货下架成功率", before: "月均 47 件人工", after: "月均 1,840 件自动", beforeEn: "47/month, manual", afterEn: "1,840/month, automated" },
      { label: "Trend-to-shelf cycle", labelZh: "趋势到上架周期", before: "26 周", after: "11 周", beforeEn: "26 weeks", afterEn: "11 weeks" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["Moments charms", "Pandora Brilliance lab-grown diamonds", "Pandora Timeless", "Disney x Pandora", "Pandora Symbolic"],
    materials: ["925 银", "14k/18k 金", "Pandora Rose 合金", "Pandora Shine 镀金", "实验室培育钻石", "再生银/金"],
    materialsEn: ["925 silver", "14k/18k gold", "Pandora Rose alloy", "Pandora Shine gold plating", "lab-grown diamonds", "recycled silver/gold"],
    myRole: "我为潘多拉亚太数字团队做了 6 周的 AI 工作流审计（访谈 33 人），然后在他们自己的审批流和预算体系里部署了 6 个代理。决策权、签发权和预算始终在潘多拉手里——我的名字不在这条签发链路上。",
    myRoleEn: "I ran a 6-week AI workflow audit for Pandora's APAC digital team (33 interviews), then deployed the 6 agents inside their own approval flows and budget lines. Decision rights, sign-off and budget stayed with Pandora throughout — my name isn't on that sign-off chain.",
    ...
    mechanicsEn: "Scope first: this was a 3-pilot-store project, not a group-wide transformation. The WeChat VIP agent worked 42,000 CRM contacts dormant for 11 months; every message was AI-drafted and signed off one by one by the China GM — in week one they only dared send 200 a day. The counterfeit agent ingested product data from 12 marketplaces; for the first two weeks the false-positive rate was too high to auto-file takedowns, and it only opened up after the confidence threshold moved from 0.75 to 0.91. The Xiaohongshu trend agent scans 50,000+ posts a week and feeds the buying team as reference — it never touches orders directly.",
    ...
    frictionEn: { problem: "The WeChat reach channel hit rate limits on day one: a mass send triggered Tencent throttling and 3 official WeChat work accounts were restricted for 48 hours — the project nearly died on day one.", fix: "We switched to queued sending — 180 messages per account per hour, all human-approved before queuing; later we moved to the mini-program service-notification channel and the throttle never triggered again." },
  },
  {
    slug: "georg-jensen",
    ...
    hqEn: "Copenhagen (now part of Finland's Fiskars Group)",
    segment: "百年奢华银器与设计珠宝",
    segmentEn: "Centuries-old luxury silverware & design jewelry",
    revenue: "约 €110-140M（行业估算）",
    revenueEn: "~€110-140M (industry estimate)",
    stores: "全球 ~2,050 个销售点 · ~28 家直营",
    storesEn: "~2,050 points of sale worldwide · ~28 own stores",
    ...
    results: [
      { label: "Archive piece sales to China buyers", labelZh: "档案作品对中国买家销售", before: "0-3 件/年", after: "47 件/年（试点 8 个月）", beforeEn: "0-3 pieces/year", afterEn: "47 pieces/year (8-month pilot)" },
      { label: "Avg. archive piece ticket", labelZh: "档案作品客单价", before: "€2,800", after: "€11,400（含来源溢价）", beforeEn: "€2,800", afterEn: "€11,400 (incl. provenance premium)" },
      { label: "Mandarin inquiry response time", labelZh: "普通话咨询响应时间", before: "48-72 小时", after: "< 90 秒", beforeEn: "48-72 hours", afterEn: "< 90 seconds" },
      { label: "Forgery flagging accuracy", labelZh: "赝品标记准确率", before: "无法识别", after: "94.2% 精准", beforeEn: "Undetectable", afterEn: "94.2% accuracy" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["Daisy — Denmark's national flower pendant", "Vivianna Torun bangles", "Möbius", "Hidden Heart", "The 1837 collection"],
    materials: ["925 银", "18k 金", "钻石", "珍珠", "珐琅", "100% 再生金"],
    materialsEn: ["925 silver", "18k gold", "diamonds", "pearls", "enamel", "100% recycled gold"],
    myRole: "我为乔治·杰生的传承档案团队做了档案数字化审计，部署了检索代理和微信验真接口。档案录入由他们的两位馆员完成——我只搭管线、做质检。",
    myRoleEn: "I did the archive digitization audit for Georg Jensen's heritage team and deployed the retrieval agent and the WeChat verification interface. The record entry itself was done by their two archivists — I built the pipeline and ran quality control.",
    ...
    mechanicsEn: "Of the 4,200 archive records, only 1,900 were structured; the other 2,300 were scans of hand-written ledgers from the last century, keyed in by the two archivists over 5 weeks. The day the vector database went live I ran 100 test questions — Mandarin answer accuracy was 87%, and nearly all of the missing 13% were silverware terminology translation errors (“hammered finish” rendered as “patterns on a hammer”). Each blockchain verification certificate costs RMB 0.9 to mint — the brand insisted on paying that themselves: it's their credibility, it shouldn't be skimped on.",
    ...
    frictionEn: { problem: "The handwritten ledgers were in 1930s Danish shorthand; OCR recognition ran at only 41%. At the end of week 1 the two archivists, averaging 62 years old, said: “This can't be finished — we want to give up.”", fix: "I split every OCR field below 70% confidence into a manual review queue: 90 minutes per person per day, a 27-inch screen, font scaled 1.5x. After 5 weeks all 2,300 records were in, with a 0.3% keying error rate." },
  },
  {
    slug: "ole-lynggaard",
    ...
    foundedEn: "1963",
    hq: "Hellerup, 哥本哈根",
    hqEn: "Hellerup, Copenhagen",
    segment: "家族高定珠宝 · 丹麦皇室御用",
    segmentEn: "Family couture jewelry · Danish royal warrant",
    revenue: "未公开 · 行业估算中双位数百万欧元",
    revenueEn: "Undisclosed · mid-double-digit millions EUR (industry estimate)",
    stores: "5 家直营旗舰 + 275 家全球零售商",
    storesEn: "5 own flagships + 275 retailers worldwide",
    ...
    results: [
      { label: "Return rate from catalog orders", labelZh: "图册下单退货率", before: "18.2%", after: "4.1%（AR 试戴后）", beforeEn: "18.2%", afterEn: "4.1% (after AR try-on)" },
      { label: "Asia inquiry conversion", labelZh: "亚洲咨询转化率", before: "3.4%", after: "21.7%", beforeEn: "3.4%", afterEn: "21.7%" },
      { label: "Bespoke design turnaround", labelZh: "高定设计交付周期", before: "12-16 周", after: "5-7 周", beforeEn: "12-16 weeks", afterEn: "5-7 weeks" },
      { label: "Wholesale door SKU accuracy", labelZh: "批发门店 SKU 准确率", before: "需人工经验", after: "92% 自动预测命中", beforeEn: "Manual expertise", afterEn: "92% automated forecast hit rate" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["The Snakes rings", "The Lotus stacking ring", "The Eye", "The Elephant", "Aurora collection (3rd-generation Sofia's design)"],
    materials: ["18k 黄/白/玫瑰金", "钻石", "碧玺", "蓝宝石", "祖母绿"],
    materialsEn: ["18k yellow/white/rose gold", "diamonds", "tourmaline", "sapphires", "emeralds"],
    myRole: "我为 Ole Lynggaard 的批发与零售团队部署了 AR 试戴管线和三语客服代理。工坊一天没扩，金匠团队没有被要求改变任何一道工序。",
    myRoleEn: "I deployed the AR try-on pipeline and the trilingual service agents for Ole Lynggaard's wholesale and retail teams. The workshop didn't grow by a single day, and the goldsmiths weren't asked to change a single step of their process.",
    ...
    mechanicsEn: "We didn't build an app for AR try-on — just a web QR code, scan and go, because not one of the 275 wholesale doors would install software for a single brand. The 3D models came straight from The Snakes CAD files; the diamond refraction parameters took 3 revisions to get close to the real thing. In the trilingual agents' first month, a Tokyo buyer flagged 2 honorific errors in the Japanese replies — since then a native Japanese reviewer does 1 hour of sampling a week, and there have been no more incidents. Bespoke stays render-only: the final design is always drawn by Ole himself.",
    ...
    frictionEn: { problem: "On iPhone front cameras the AR ring would “float” 2-3mm above the finger bone. The pilot store manager's exact words: “It looks fake — I don't dare show it to customers.”", fix: "Added finger-bone contour detection and gravity alignment; the float went from 3mm down to 0.4mm. The day the manager re-signed off, he opened a bottle of champagne." },
  },
  {
    ...
    hqEn: "Copenhagen",
    segment: "奢华定制 · 灵性珠宝",
    segmentEn: "Luxury bespoke · spiritual jewelry",
    revenue: "未公开 · 单位数百万欧元（行业估算）",
    revenueEn: "Undisclosed · single-digit millions EUR (industry estimate)",
    stores: "哥本哈根旗舰 + 全球精选奢侈品多品牌店",
    storesEn: "Copenhagen flagship + selected luxury multi-brand stores worldwide",
    ...
    results: [
      { label: "China bespoke orders / quarter", labelZh: "中国定制订单/季度", before: "3-5 件", after: "31 件", beforeEn: "3-5 pieces", afterEn: "31 pieces" },
      { label: "Avg. bespoke ticket from China", labelZh: "中国定制客单价", before: "€9,200", after: "€14,800（多珠组合）", beforeEn: "€9,200", afterEn: "€14,800 (multi-bead builds)" },
      { label: "Counterfeit takedowns / month", labelZh: "假货下架/月", before: "12 件人工", after: "1,240 件自动", beforeEn: "12/month, manual", afterEn: "1,240/month, automated" },
      { label: "Trunk-show cost as % of revenue", labelZh: "巡展成本占营收比", before: "22%", after: "8%", beforeEn: "22%", afterEn: "8%" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["The classic Shamballa Bracelet", "SHAMBALLA Lock Bracelet", "The Non-Braided collection", "The Star collection", "One-of-a-kind bespoke pieces"],
    materials: ["18k 黄/白/红金", "钻石", "蓝宝石/红宝石/祖母绿", "沙弗莱石", "黑色陶瓷"],
    materialsEn: ["18k yellow/white/red gold", "diamonds", "sapphires/rubies/emeralds", "tsavorite garnet", "black ceramic"],
    myRole: "我为 Shamballa 部署了微信礼宾代理和 AR 手链配置器的原型。Mads 亲自训练代理讲了 6 个下午——佛教象征那一部分，我一个字没碰。",
    myRoleEn: "I prototyped Shamballa's WeChat concierge agent and the AR bracelet configurator. Mads personally trained the agent across 6 afternoons — on the Buddhist symbolism, I didn't touch a single word.",
    ...
    mechanicsEn: "Mads dictated 9 hours of design rules, which we distilled into 214 constraints hard-coded into the agent. Of the configurator's 4,200 first-month builds, only 310 (7.4%) turned into price inquiries — the team was briefly deflated. Then we saw that high-net-worth clients follow a “talk first, configure second” path; after reordering the flow, the inquiry rate hit 14.1%. Anti-counterfeit only covers Taobao and Pinduoduo for now: Douyin's complaint channel requires corporate credentials and took 6 weeks to unlock.",
    ...
    frictionEn: { problem: "The concierge's first brand-symbolism conversation conflated the Shamballa brand with the Tibetan Buddhist pure land of Shambhala. The client was a Buddhist scholar and corrected it to its face — Mads was a little embarrassed.", fix: "Mads spent two weekends writing a 47-item “say this, never that” boundary list, hard-coded into the agent: spiritual statements may only quote Mads verbatim — the AI translates, it never improvises." },
  },
  {
    slug: "jane-konig",
    ...
    foundedEn: "1986",
    hq: "哥本哈根市中心工坊",
    hqEn: "Workshop in central Copenhagen",
    segment: "当代丹麦高定珠宝 · 创始人主导",
    segmentEn: "Contemporary Danish couture jewelry · founder-led",
    revenue: "未公开 · 单位数百万欧元（行业估算）",
    revenueEn: "Undisclosed · single-digit millions EUR (industry estimate)",
    stores: "哥本哈根工坊 + 北欧/欧洲多品牌店",
    storesEn: "Copenhagen workshop + Nordic/European multi-brand stores",
    ...
    results: [
      { label: "New designs approved per year", labelZh: "年度批准新设计数", before: "120 件", after: "340 件（Jane 仅审核）", beforeEn: "120 pieces", afterEn: "340 pieces (Jane reviews only)" },
      { label: "Xiaohongshu follower growth", labelZh: "小红书粉丝增长", before: "0", after: "18,400（6 个月）", beforeEn: "0", afterEn: "18,400 (6 months)" },
      { label: "China wholesale inquiries / month", labelZh: "中国批发咨询/月", before: "0-1", after: "27", beforeEn: "0-1", afterEn: "27" },
      { label: "Jane's design time per piece", labelZh: "Jane 每件设计时间", before: "8-12 小时", after: "1-2 小时审核", beforeEn: "8-12 hours", afterEn: "1-2 hours of review" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["Reflection Heart necklace", "Hoop with Stone earrings", "Half Moon earrings", "Twist", "The Lulu collection"],
    materials: ["14k 金", "18k 金", "925 银", "宝石"],
    materialsEn: ["14k gold", "18k gold", "925 silver", "gemstones"],
    myRole: "我为 Jane Kønig 的工作室部署了设计辅助管线和小红书内容代理。Jane 是唯一的审批人——没有她的签字，任何产出不出门。",
    myRoleEn: "I deployed the design-assist pipeline and the Xiaohongshu content agent for Jane Kønig's studio. Jane is the only approver — nothing ships without her signature.",
    ...
    mechanicsEn: "One bespoke hand sketch used to take Jane 8 hours. We trained a LoRA model on her past 1,400 sketches; AI now drafts 5 initial options in 4 minutes and Jane polishes for 20. Annual output went from 120 to 340 pieces with zero dilution of her design DNA. For the first 3 months she personally reviewed all 4 Xiaohongshu posts a week; in month 4 she delegated post review to the studio manager and kept only final design approval.",
    ...
    frictionEn: { problem: "Early on the AI would hallucinate gemstone carat weights and generate physically impossible rings — one version set a 3-carat stone in 1.2mm prongs.", fix: "We added a parameter lock at the Bench Automation layer (in plain terms, a machine checkpoint on AI output): before final CAD, every proposal must validate against the master gemstone inventory — mismatched stone weight, prong diameter or setting tolerance gets bounced straight back for regeneration." },
  },
  {
    ...
    segmentEn: "Accessible Nordic everyday jewelry · founder signing off",
    revenue: "未公开 · 已宣布 2026 年关停",
    revenueEn: "Undisclosed · closing announced for 2026",
    stores: "无直营 · 北欧批发 + 全球 DTC",
    storesEn: "No own stores · Nordic wholesale + global DTC",
    ...
    results: [
      { label: "Closing-sale revenue recovery", labelZh: "清仓营收回收", before: "预计 €220K（标准折扣）", after: "€478K（动态定价）", beforeEn: "€220K projected (standard discounting)", afterEn: "€478K (dynamic pricing)" },
      { label: "Archive IP valuation range", labelZh: "档案 IP 估值区间", before: "未估值", after: "€840K-1.2M", beforeEn: "Unvalued", afterEn: "€840K-1.2M" },
      { label: "Reissue candidates identified", labelZh: "识别的再发行候选", before: "0", after: "23 件常青设计", beforeEn: "0", afterEn: "23 evergreen designs" },
      { label: "Acquisition deal timeline", labelZh: "收购交易时间线", before: "18 个月（典型）", after: "90 天（数据室就绪）", beforeEn: "18 months (typical)", afterEn: "90 days (data room ready)" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["Amour pendant", "Nova Choker", "Bay Pearl Ring", "Poetry Ring", "Love Letter Pendant"],
    materials: ["925 银", "14k/18k 镀金", "淡水珍珠"],
    materialsEn: ["925 silver", "14k/18k gold plating", "freshwater pearls"],
    myRole: "我为 Pernille Corydon 的清盘做了库存审计、动态定价部署和收购数据室准备。她付固定顾问费——估值没有分成，我没有任何理由把数字往高了说。",
    myRoleEn: "I handled the inventory audit, dynamic pricing deployment and acquisition data room for the wind-down. She paid a flat advisory fee — no percentage of the valuation, so I had zero incentive to inflate the number.",
    ...
    mechanicsEn: "For 412 of the 1,847 historical SKUs the purchase-cost records were missing and had to be back-derived from collection averages — which is why the archive IP valuation is a wide €840K-1.2M range rather than a falsely precise single number. Dynamic repricing runs 4x a day on a simple rule: inventory depth × days to close × 7-day rolling sell-through. In week 3 of the data room we hit a landmine: two 2019 KOL collaboration agreements had ambiguous content-IP ownership; written confirmation took 3 more weeks.",
    ...
    frictionEn: { problem: "On day 2 of dynamic pricing, a longtime customer found the bracelet she'd bought the week before had dropped 30% and left a 1-star Trustpilot review: “The brand is punishing loyal customers who paid early.”", fix: "Added a 14-day price guarantee: any price drop during the closing sale auto-refunds the difference — 214 orders refunded, €6,300 total. That 1-star later became 5 stars, reading “refund on price drops. Classy.”" },
  },
  {
    slug: "maanesten",
    brandName: "Maanesten",
    ...
    segmentEn: "Sustainable fashion jewelry · moonstone spirituality",
    revenue: "未公开 · 2024 国际收缩后重新聚焦北欧",
    revenueEn: "Undisclosed · refocused on the Nordics after the 2024 international retreat",
    stores: "首店 2018 · 北欧 + 德国 + 美国 DTC",
    storesEn: "First store 2018 · Nordic + Germany + US DTC",
    ...
    results: [
      { label: "Markets ruled out (saved cost)", labelZh: "排除的市场（节省成本）", before: "4 个候选", after: "0 个被否决 → 4 个明确否决", beforeEn: "4 candidates", afterEn: "0 blocked → 4 definitively ruled out" },
      { label: "Forecast accuracy (90-day SKU)", labelZh: "90 天 SKU 预测准确率", before: "未预测", after: "87.4%", beforeEn: "No forecasting", afterEn: "87.4%" },
      { label: "Content cost per market", labelZh: "每市场内容成本", before: "€18K/月（本地代理）", after: "€900/月（AI 引擎）", beforeEn: "€18K/month (local agency)", afterEn: "€900/month (AI engine)" },
      { label: "Customer LTV segmentation accuracy", labelZh: "客户 LTV 细分准确率", before: "RFM 静态", after: "动态 8%/60% 识别", beforeEn: "Static RFM", afterEn: "Dynamic 8%/60% identification" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["Sundance charms", "Seasonal moonstone collections", "Earrings", "Necklaces", "Rings"],
    materials: ["SGS 认证再生 925 银", "18k 镀金", "月光石", "半宝石"],
    materialsEn: ["SGS-certified recycled 925 silver", "18k gold plating", "moonstone", "semi-precious stones"],
    myRole: "我为 Maanesten 的董事会做了市场再入场的数据审计，并部署了需求预测代理。去不去、去哪、投多少——决定权从头到尾在董事会。",
    myRoleEn: "I ran the data audit for Maanesten's board on market re-entry and deployed the demand-forecasting agent. Whether to go, where, and how much to invest — the decision stayed with the board from start to finish.",
    ...
    mechanicsEn: "9 of the 38 demand signals we built ourselves (e.g. week-over-week Xiaohongshu search velocity for “moonstone”); the rest came from public sources. The 14 candidate markets produced a recommendation of only 2 — the board had wanted to enter 6. That “what not to do” list was the real deliverable of this engagement. First-month accuracy of the 90-day demand forecast was just 71.2%; it climbed to 87.4% only after we added raw-moonstone lead time as a variable.",
    ...
    frictionEn: { problem: "At the first board presentation a founding shareholder said on the spot: “Your data just vetoed three markets I opened with my own hands in 2019.” The meeting deadlocked for 20 minutes.", fix: "We broke the veto logic into a 38-item scoring sheet and invited him to reweight each item himself — after his reweighting, those 3 markets still ranked last. The data hadn't changed, but the conclusion became “ours jointly”." },
  },
  {
    ...
    hqEn: "Copenhagen",
    segment: "珐琅工艺珠宝 · 手工烧制",
    segmentEn: "Enamel-craft jewelry · hand-fired",
    revenue: "未公开 · 单位数百万欧元（行业估算）",
    revenueEn: "Undisclosed · single-digit millions EUR (industry estimate)",
    stores: "无直营 · 北欧/欧洲批发 + DTC 网店",
    storesEn: "No own stores · Nordic/European wholesale + DTC webshop",
    chinaStatus: "No documented presence; enamel craft resonates with Chinese cloisonné (景泰蓝) heritage",
    ...
    results: [
      { label: "Seasonal color designs generated", labelZh: "季节色彩设计生成数", before: "12/季（Marie 手工）", after: "48 候选/季（Marie 审核 18）", beforeEn: "12/season (Marie by hand)", afterEn: "48 candidates/season (Marie approves 18)" },
      { label: "Online return rate (color mismatch)", labelZh: "在线退货率（色差）", before: "23%", after: "5.2%（AR 试戴后）", beforeEn: "23%", afterEn: "5.2% (after AR try-on)" },
      { label: "Misfired batch losses", labelZh: "窑烧失误损失", before: "€52K/年", after: "€8K/年（预测准确）", beforeEn: "€52K/year", afterEn: "€8K/year (accurate forecasting)" },
      { label: "China Xiaohongshu engagement", labelZh: "中国小红书互动", before: "0", after: "6,200 收藏/月（景泰蓝叙事）", beforeEn: "0", afterEn: "6,200 saves/month (cloisonné storytelling)" },
    ],
    timeline: [
      ...
      { week: "Week 7-10", title: "Mandarin storytelling", titleZh: "普通话叙事", detail: "First 30 Xiaohongshu posts published; cloisonné (景泰蓝) resonance tracked.", detailZh: "发布首批 30 条小红书帖子；追踪景泰蓝共鸣。" },
    ...
    keyCollectionsEn: ["Enamel pendant necklaces", "Enamel earrings", "Enamel rings", "Seasonal color collections"],
    materials: ["玻璃珐琅", "925 银", "14k/18k 镀金"],
    materialsEn: ["Vitreous enamel", "925 silver", "14k/18k gold plating"],
    myRole: "我为 ENAMEL 哥本哈根部署了色彩生成辅助和 AR 色彩校准管线。窑还是 Marie 的窑，每一窑烧什么由她决定。",
    myRoleEn: "I deployed the color-generation assist and the AR color-calibration pipeline for ENAMEL Copenhagen. The kiln is still Marie's kiln — what gets fired in each batch is her call.",
    ...
    mechanicsEn: "The training set is Marie's 800 archive designs + 1,200 historical European enamel references, and every generation passes a same-temperature firing check first — cobalt blue and strontium yellow can't share a kiln (0.8% shrink-rate difference); that rule was paid for with 11 years of Marie's failed firings. The AR color calibration is a mapping table built from 200 color-chart photos on each of 6 phones at different price points — the 94% color fidelity is the mean over that test set, not a promise.",
    ...
    frictionEn: { problem: "Xiaohongshu users on budget Android phones posted real-product photos that visibly mismatched the AR rendering (ΔE 8.3, far beyond acceptable), and the comments started filling with “photo scam”.", fix: "We tiered the mapping table by device model — low-end phones get a more conservative color-compression curve — and pinned one line on every product page: “Screen rendering is indicative; a physical color card is available on request.” Returns held at 5.2% and the accusation posts stopped." },
  },
  {
    slug: "trollbeads",
    ...
    segmentEn: "Original beads · Murano glass · collectors",
    revenue: "未公开 · 行业估算中双位数百万欧元",
    revenueEn: "Undisclosed · mid-double-digit millions EUR (industry estimate)",
    stores: "哥本哈根旗舰 + 全球多品牌零售",
    storesEn: "Copenhagen flagship + global multi-brand retail",
    ...
    results: [
      { label: "Avg. collector spend / year", labelZh: "收藏家年度平均消费", before: "€180", after: "€620（推荐驱动）", beforeEn: "€180", afterEn: "€620 (recommendation-driven)" },
      { label: "Limited-drop sell-through", labelZh: "限量发售售罄率", before: "未做限量", after: "94% 在 72 小时内", beforeEn: "No limited drops", afterEn: "94% within 72 hours" },
      { label: "China collector base growth", labelZh: "中国收藏家增长", before: "未追踪", after: "4,800 活跃收藏家（8 个月）", beforeEn: "Untracked", afterEn: "4,800 active collectors (8 months)" },
      { label: "Repeat purchase rate", labelZh: "复购率", before: "31%", after: "68%", beforeEn: "31%", afterEn: "68%" },
    ],
    timeline: [
      ...
      { week: "Week 11-14", title: "Culture-creative (文创) China entry", titleZh: "文创中国切入", detail: "Xiaohongshu + Douyin launch; first 2,000 collectors onboarded.", detailZh: "小红书 + 抖音上线；首批 2,000 收藏家接入。" },
    ],
    agents: ["Collector Recommendation Agent", "Limited-Drop Design Agent", "Murano Provenance Agent", "Culture-Creative (文创) China Entry Agent"],
    ...
    keyCollectionsEn: ["The original “Mask” bead (1976)", "Murano glass beads", "The X-Zone collection", "Unique one-of-a-kinds", "Seasonal limited editions"],
    materials: ["Murano 玻璃", "925 银", "18k 金", "宝石"],
    materialsEn: ["Murano glass", "925 silver", "18k gold", "gemstones"],
    myRole: "我为 Trollbeads 的电商团队做了收藏家数据审计和推荐系统部署。12 年的订单数据一直在他们手里——我只是让它能说话。",
    myRoleEn: "I ran the collector data audit and deployed the recommendation system for Trollbeads' e-commerce team. Twelve years of order data had been sitting in their hands all along — I just made it talk.",
    ...
    mechanicsEn: "31% of the 12-year purchase history couldn't be identity-merged (veteran collectors who changed email 3 times are common); data cleaning took 4 weeks. The recommendation logic isn't mysterious: collection completeness + missing zodiac/birthstone + price-band jump warning (going from a €60 bead to a €600 bead needs runway). The first 8,000 recommendations got a 22.6% click rate but only 3.1% conversion; when the copy changed from “your next bead” to “the one you're missing”, conversion hit 6.8%.",
    ...
    frictionEn: { problem: "In week 3 of the Murano provenance certificates, an Italian artisan found his name misspelled on a certificate and used the words “legal action” in his email.", fix: "The certificate flow now has the artisan confirm spelling and signature on video; the 74 affected certificates were re-minted at our cost. The artisan later left a blessing in the recommendation system's staging environment." },
  },
  {
    ...
    foundedEn: "2011",
    hq: "哥本哈根 + 纽约麦迪逊大道",
    hqEn: "Copenhagen + Madison Avenue, New York",
    segment: "高定珠宝 · 现代传家宝 · 宇宙灵感",
    segmentEn: "Fine jewelry · modern heirlooms · cosmic inspiration",
    revenue: "未公开 · 行业估算低双位数百万欧元（快速增长）",
    revenueEn: "Undisclosed · low-double-digit millions EUR (industry estimate, fast-growing)",
    stores: "2 家直营（哥本哈根 + 纽约）+ Harrods/Maxfield 等顶级精品",
    storesEn: "2 own stores (Copenhagen + New York) + top retailers incl. Harrods/Maxfield",
    ...
    results: [
      { label: "China revenue in Year 1", labelZh: "中国首年营收", before: "0", after: "€1.8M（试点 6 个月）", beforeEn: "0", afterEn: "€1.8M (6-month pilot)" },
      { label: "Xiaohongshu followers", labelZh: "小红书粉丝", before: "0", after: "47,200（6 个月）", beforeEn: "0", afterEn: "47,200 (6 months)" },
      { label: "High Jewelry waitlist applications", labelZh: "高定候补申请", before: "0", after: "340（首批 12 件配额）", beforeEn: "0", afterEn: "340 (for the first 12-piece allocation)" },
      { label: "Avg. ticket from China clients", labelZh: "中国客户客单价", before: "0", after: "€4,200", beforeEn: "0", afterEn: "€4,200" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["Croissant de Lune ear climbers", "Pearl & diamond spiral collection", "Freshwater pearl collection", "High Jewelry debut collection"],
    materials: ["18k 金", "淡水珍珠", "钻石（越来越多实验室培育）"],
    materialsEn: ["18k gold", "freshwater pearls", "diamonds (increasingly lab-grown)"],
    myRole: "我为 Sophie Bille Brahe 做了中国市场切入的数据研究和代理部署。KOL 名单 Sophie 一位一位亲自批，谈判和签约全部由她的团队完成。",
    myRoleEn: "I ran the China-entry market research and agent deployment for Sophie Bille Brahe. Sophie approved the KOL list one name at a time; all negotiation and contracting was done by her team.",
    ...
    mechanicsEn: "The agent handled the first pass on 4,200 KOLs; the heaviest-weighted of the 14 dimensions was “quiet-luxury fit of previously partnered brands” — the scoring rubric was dictated by Sophie over 3 hours and written up by me. The Tmall Luxury Pavilion onboarding materials went through 5 review rounds; the brand insisted on deleting every trace of the “cold-luxury” media label. The High Jewelry waitlist was built fully transparent: 340 applications, how the 12 allocations were decided, where you stand in line — applicants can look it all up themselves.",
    ...
    frictionEn: { problem: "In week 5 the Xiaohongshu account got throttled for 10 days — a new account's posting frequency tripped the platform's risk controls and the entire content calendar was scrapped.", fix: "We dropped to 2 posts a week and built a KOC repost matrix to distribute reach. After the throttle lifted, follower growth was actually faster than before: Xiaohongshu's algorithm rewards “genuine engagement” — fast isn't always good." },
  },
  {
    slug: "maria-black",
    ...
    hqEn: "Copenhagen (Silkegade 13)",
    segment: "雕塑珠宝 + 奢华穿刺工作室",
    segmentEn: "Sculptural jewelry + luxury piercing studios",
    revenue: "未公开 · 行业估算低双位数百万欧元",
    revenueEn: "Undisclosed · low-double-digit millions EUR (industry estimate)",
    stores: "5 家直营工作室：哥本哈根/奥胡斯/柏林/奥斯陆/东京",
    storesEn: "5 own studios: Copenhagen/Aarhus/Berlin/Oslo/Tokyo",
    ...
    results: [
      { label: "Shanghai flagship opening timeline", labelZh: "上海旗舰开业时间线", before: "未启动", after: "16 周（手册就绪）", beforeEn: "Not started", afterEn: "16 weeks (playbook ready)" },
      { label: "Appointment no-show rate (Tokyo pilot)", labelZh: "爽约率（东京试点）", before: "14%", after: "3.2%（AI 随访后）", beforeEn: "14%", afterEn: "3.2% (after AI follow-ups)" },
      { label: "Aftercare 5-star reviews", labelZh: "术后 5 星评价", before: "76%", after: "94%", beforeEn: "76%", afterEn: "94%" },
      { label: "Piercing service revenue / piercer / day", labelZh: "每位穿刺师日营收", before: "€1,240", after: "€1,890（动态定价）", beforeEn: "€1,240", afterEn: "€1,890 (dynamic pricing)" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["The Stellar collection", "Coil Huggies", "Seasonal silver collections", "14k permanent-jewelry piercing line", "Lab-grown diamond piercings"],
    materials: ["100% 再生 925 银", "14k 金", "实验室培育钻石"],
    materialsEn: ["100% recycled 925 silver", "14k gold", "lab-grown diamonds"],
    myRole: "我为 Maria Black 做了上海旗舰店的选址数据模型，并部署了预约与术后随访代理。租约是他们的团队谈的——我只是把数据摆在桌上。",
    myRoleEn: "I built the site-selection data model for Maria Black's Shanghai flagship and deployed the booking and aftercare agents. The lease was negotiated by their team — I just put the data on the table.",
    ...
    mechanicsEn: "6 candidate sites × 18 dimensions: Xintiandi ranked #1 not on foot traffic (Huaihai Road has more) but on a 41% Gen-Z share and piercing-friendly property permits. One hard rule in the booking agent: after complex piercings (bridge, tongue) a mandatory 45-minute buffer — none of the 4 piercers can be double-booked past it. Aftercare messages go out on days 1/3/7/14/30 via WeChat; the copy was rewritten line by line by the two Tokyo piercers.",
    ...
    frictionEn: { problem: "The WeChat mini-program booking component sat in review for 11 days — “piercing” was rejected on first submission as a sensitive category, with no word on which line offended.", fix: "We recategorized as “custom jewelry service”, renamed the service “wearing consultant”, and added one line — “this service is not a medical procedure”. Approved on day 12; the business itself didn't change by a single word." },
  },
  {
    slug: "pilgrim",
    brandName: "PILGRIM",
    ...
    foundedEn: "1983",
    hq: "Skanderborg, 丹麦",
    hqEn: "Skanderborg, Denmark",
    segment: "accessible 时尚珠宝 · 音乐节起源 · 二代接班",
    segmentEn: "Accessible fashion jewelry · festival roots · 2nd-generation leadership",
    revenue: "未公开 · 行业估算中双位数百万欧元",
    revenueEn: "Undisclosed · mid-double-digit millions EUR (industry estimate)",
    stores: "北欧 + 欧洲零售足迹 + 加拿大扩张",
    storesEn: "Nordic + European retail footprint + Canada expansion",
    ...
    results: [
      { label: "Active SKU count", labelZh: "活跃 SKU 数", before: "1,847", after: "1,167（淘汰 680）", beforeEn: "1,847", afterEn: "1,167 (680 retired)" },
      { label: "Working capital released", labelZh: "释放流动资金", before: "0", after: "€420K", beforeEn: "0", afterEn: "€420K" },
      { label: "Design cycle time", labelZh: "设计周期", before: "16 周", after: "6 周", beforeEn: "16 weeks", afterEn: "6 weeks" },
      { label: "Inventory write-down", labelZh: "库存减值", before: "€680K/年", after: "€180K/年", beforeEn: "€680K/year", afterEn: "€180K/year" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["Timeless basics", "Refined fashion pieces", "Seasonal collections", "Sunglasses line", "In-store piercing service"],
    materials: ["100% 再生 925 银", "18k 镀金", "镍安全", "防水"],
    materialsEn: ["100% recycled 925 silver", "18k gold plating", "nickel-safe", "waterproof"],
    myRole: "我为 PILGRIM 做了 SKU 数据审计和设计加速管线的部署。淘汰哪 680 个 SKU 是采销团队一票一票投出来的——模型只提供依据，不投票。",
    myRoleEn: "I ran the SKU data audit and deployed the design-acceleration pipeline for PILGRIM. Which 680 SKUs to retire was voted one ballot at a time by the buying team — the model supplies the evidence, it doesn't vote.",
    ...
    mechanicsEn: "The 1,847 SKUs' sales data was scattered across 3 systems (Shopify, a 2016 ERP, and Excel); reconciling took 3 weeks. The design agent trained on 25 years of archive + Dea's 47 new-direction sketches produces 60 candidates a cycle — but her adoption rate was 8% for the first two cycles; her feedback: “too much like old PILGRIM”. After we tripled the sample weight on the new direction, adoption hit 33% and she started actually using it.",
    ...
    frictionEn: { problem: "When the retirement list went public, a wholesaler of 14 years found 41 of the 60 styles he carries on it and called the CEO directly.", fix: "We added a 90-day transition protection window for wholesalers, executed the list in two waves, and used part of the €420K released working capital to extend payment terms for core wholesalers — the relationship survived, the SKUs still got cut." },
  },
  {
    slug: "hartmanns",
    ...
    foundedEn: "Copenhagen jeweler heritage (year TBC)",
    hq: "哥本哈根",
    hqEn: "Copenhagen",
    segment: "高端钻石珠宝 · 格陵兰红宝石独家",
    segmentEn: "High-end diamond jewelry · Greenland ruby exclusive",
    revenue: "未公开 · 行业估算低双位数百万欧元",
    revenueEn: "Undisclosed · low-double-digit millions EUR (industry estimate)",
    stores: "哥本哈根旗舰（Slettvoll 设计）",
    storesEn: "Copenhagen flagship (Slettvoll design)",
    ...
    results: [
      { label: "China HNW client inquiries / month", labelZh: "中国高净值客户咨询/月", before: "0-2", after: "38", beforeEn: "0-2", afterEn: "38" },
      { label: "Avg. Greenland-ruby ticket", labelZh: "格陵兰红宝石客单价", before: "€8,400", after: "€24,200（来源溢价）", beforeEn: "€8,400", afterEn: "€24,200 (provenance premium)" },
      { label: "Undisclosed-treatment fraud caught", labelZh: "未披露处理欺诈捕获", before: "0", after: "3 起供应商欺诈（早期识别）", beforeEn: "0", afterEn: "3 supplier frauds (caught early)" },
      { label: "Bespoke design approval cycle", labelZh: "高定设计批准周期", before: "12 周（多次实地访问）", after: "3 周（AR 可视化）", beforeEn: "12 weeks (multiple in-person visits)", afterEn: "3 weeks (AR visualization)" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["Diamond high jewelry", "Bespoke commissions", "The Greenland ruby exclusive collection"],
    materials: ["精品钻石", "格陵兰红宝石", "金", "铂金"],
    materialsEn: ["Fine diamonds", "Greenland rubies", "gold", "platinum"],
    myRole: "我为 Hartmann's 部署了红宝石来源存证管线和微信验真小程序。GIA/GRS 证书数据由他们的宝石实验室提供——我只负责链上存证和读取。",
    myRoleEn: "I deployed the ruby provenance attestation pipeline and the WeChat verification mini-program for Hartmann's. The GIA/GRS certificate data comes from their gem lab — I'm responsible for on-chain attestation and reading only.",
    ...
    mechanicsEn: "Each Greenland ruby's attestation includes: mine license block coordinates, GIA/GRS report number, spectrometer data fingerprint, and rough-stone photo. Minting one on-chain certificate costs RMB 1.4 — cheaper than you'd think; the expensive part was standardizing spectrometer data, with 3 suppliers on 3 incompatible formats: 5 weeks to unify. The WeChat verification mini-program was scanned 1,102 times in its first month — 89 of those were people verifying stones bought on the secondary market as “suspected fakes”.",
    ...
    frictionEn: { problem: "In its first week the treatment-detection agent flagged 2 genuinely unheated rubies as heat-treated (spectral baseline drift); a supplier email asked “are your machines even reliable?”", fix: "The baseline now calibrates dynamically per mine and batch, and the threshold was downgraded from “auto-block” to “flag for human re-check” — better a day slower than one stone wrong." },
  },
  {
    slug: "aurum-denmark",
    ...
    foundedEn: "Recent DTC startup (inspired by a grandfather's note)",
    hq: "丹麦（在线优先 + 集成工坊）",
    hqEn: "Denmark (online-first + integrated workshop)",
    segment: "可持续 DTC 高定珠宝 · 故事驱动",
    segmentEn: "Sustainable DTC fine jewelry · story-driven",
    revenue: "未公开 · 微型 DTC 初创",
    revenueEn: "Undisclosed · micro DTC startup",
    stores: "在线专营（aurumdenmark.com）· 全球配送",
    storesEn: "Online only (aurumdenmark.com) · ships worldwide",
    ...
    results: [
      { label: "Memory-jewelry orders / month", labelZh: "记忆珠宝订单/月", before: "8 件（创始人手工）", after: "62 件（AI + 创始人审核）", beforeEn: "8 pieces (made by the founder)", afterEn: "62 pieces (AI + founder review)" },
      { label: "Xiaohongshu follower growth", labelZh: "小红书粉丝增长", before: "0", after: "11,800（5 个月）", beforeEn: "0", afterEn: "11,800 (5 months)" },
      { label: "Avg. ticket", labelZh: "客单价", before: "€340", after: "€890（记忆珠宝溢价）", beforeEn: "€340", afterEn: "€890 (memory-jewelry premium)" },
      { label: "Blockchain-verified pieces", labelZh: "区块链验证作品", before: "0", after: "100%（每件都有公开证书）", beforeEn: "0", afterEn: "100% (public certificate for every piece)" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["The Forget-Me-Not necklace (origin piece)", "The Luna necklace", "Client memory bespoke collection"],
    materials: ["100% 再生金", "实验室培育钻石（RJC 成员）", "桑皮纸包装"],
    materialsEn: ["100% recycled gold", "lab-grown diamonds (RJC members)", "Mulberry paper packaging"],
    myRole: "我为 Aurum 部署了记忆珠宝配置器和公开链存证。他们团队一共 3 个人——我的报价也按 3 个人的规模砍了一半。",
    myRoleEn: "I deployed the memory-jewelry configurator and the public-chain attestation for Aurum. Their whole team is 3 people — my quote got cut in half to match.",
    ...
    mechanicsEn: "The configurator takes photo, text and voice input; the 4th input, “handwritten note”, came from a client two weeks after launch — all she had was her grandmother's note in 1960s Danish handwriting that OCR couldn't read, so the founder traced it herself. The founder reviews all designs on Sunday evenings, averaging 40 minutes for 18 pieces. Public-chain attestation costs RMB 0.3 per piece — the only price point that's friendly to a micro brand.",
    ...
    frictionEn: { problem: "Voice input raised a privacy question: a client uploaded a call recording with her late mother, and the team couldn't say whether that audio counted as personal data or could enter the training set.", fix: "We set the rule on the spot: recordings are used for generation only, never for training, physically deleted after 72 hours — plus a client-facing data-processing note. That necklace later became the brand's most-shared piece on Xiaohongshu." },
  },
  {
    ...
    foundedEn: "Journey began in 1990",
    hq: "Frederiksberg, 哥本哈根（Kingosgade 12）",
    hqEn: "Frederiksberg, Copenhagen (Kingosgade 12)",
    segment: "boutique 高定工坊 · 个性化定制",
    segmentEn: "Boutique couture atelier · bespoke commissions",
    revenue: "未公开 · 单一工坊微型（行业估算 < €1M）",
    revenueEn: "Undisclosed · single-atelier micro (< €1M, industry estimate)",
    stores: "1 家 Frederiksberg 工坊",
    storesEn: "1 atelier in Frederiksberg",
    ...
    results: [
      { label: "Inquiries answered / month", labelZh: "回复咨询/月", before: "12（其余被忽略）", after: "100%（所有咨询 24 小时内回应）", beforeEn: "12 (the rest ignored)", afterEn: "100% (every inquiry answered within 24 hours)" },
      { label: "Bespoke commissions accepted", labelZh: "接受的定制委托", before: "30/年", after: "44/年（更高吞吐）", beforeEn: "30/year", afterEn: "44/year (higher throughput)" },
      { label: "Avg. design approval cycle", labelZh: "平均设计批准周期", before: "4-6 周多次会面", after: "10-14 天（3D 渲染）", beforeEn: "4-6 weeks, multiple meetings", afterEn: "10-14 days (3D renders)" },
      { label: "Archive digitized", labelZh: "档案数字化", before: "0 件", after: "1,247 件（36 年）", beforeEn: "0 pieces", afterEn: "1,247 pieces (36 years)" },
    ],
    timeline: [
    ...
    keyCollectionsEn: ["Bespoke engagement rings", "Anniversary commissions", "Family heirloom commissions"],
    materials: ["金", "宝石", "传统手工金工"],
    materialsEn: ["gold", "gemstones", "traditional hand goldsmithing"],
    myRole: "我为 Charlotte Larsen 搭了接单简报、设计渲染和档案三条自动化管线。她 71 岁——学习成本必须趋近于零，所有新东西都长在她已有的工作习惯里。",
    myRoleEn: "I built three automation pipelines for Charlotte Larsen — intake briefs, design rendering, and archive. She's 71 — the learning cost had to approach zero, so everything new lives inside habits she already has.",
    ...
    mechanicsEn: "The 36-year archive digitization was done over one summer by her granddaughter (a design student): 1,247 pieces, each photographed with handwritten labels transcribed. The render agent works as “Charlotte sketches → AI colors and renders”, not generation — her first reaction was “I don't let a machine draw my lines”. The intake agent turns every inquiry into an 11-field standard brief; she only replies “accept” or “decline” — and even the declines get a reply within 24 hours with alternative suggestions.",
    ...
    frictionEn: { problem: "In week 9 of archive scanning we found 3 client sketches from the 1990s marked “do not copy” — scanning them into the retrieval base could violate the original agreements.", fix: "Those 3 pieces were locked into a “legal use only” directory outside the retrieval base; the remaining 1,244 pieces each got a written client consent letter before entering search." },
  },
  {
    slug: "zhuyun-nansha",
    ...
    foundedEn: "2015",
    hq: "广州南沙自贸区 · 粤港澳大湾区",
    hqEn: "Nansha FTZ, Guangzhou · Greater Bay Area",
    segment: "OEM 转型自主品牌 · 珠江三角洲制造基因",
    segmentEn: "OEM-to-own-brand pivot · Pearl River Delta manufacturing DNA",
    revenue: "约 8-12 亿人民币/年（OEM 业务为主，行业估算）",
    revenueEn: "~RMB 800M-1.2B/yr (mainly OEM, industry estimate)",
    stores: "OEM 出货至欧洲 30+ 品牌 · 2024 启动自主品牌零售",
    storesEn: "OEM to 30+ European brands · own-brand retail from 2024",
    ...
    challenge:
      "Zhuyun Jewelry was founded in 2015 in the Nansha Free Trade Zone, Guangzhou, with its factory in Panyu — China's jewelry manufacturing capital. For 15 years it has done OEM manufacturing for 30+ European jewelry brands (including several Buster served in Denmark). The manufacturing craft is world-class: 18k micro-pavé, enamel firing and diamond pavé all meet top European brand standards. But the 2024 decision to pivot to its own brand “Zhuyun” (珠韵 — “the rhyme of the Pearl River”) is a steep one: OEM thinking ≠ brand thinking. Manufacturing efficiency is world-class, but brand positioning, Xiaohongshu content, Douyin livestreaming, WeChat private domain and client management all start from zero. More critically: the European OEM clients can't be dropped (80% of revenue), so the own brand must launch in parallel without disturbing the existing business. The board's target: own-brand revenue past RMB 100M within 18 months — traditional consultancies' plans said 36.",
    challengeZh:
    ...
    solution:
      "I brought the BHAI method from Denmark with 4 key adaptations for the Chinese context: (1) all client data deployed on Aliyun's Shanghai region, triple-compliant with PIPL + Data Security Law + Cybersecurity Law — only anonymized pattern data flows back to my Copenhagen control center; (2) the AI agents connect to both international models (Claude) and domestic models (Qwen, ERNIE), client's choice; (3) I translated the methodology from the 15 Danish cases — especially Sophie Bille Brahe's quiet-luxury entry, Pandora's anti-counterfeit authentication, Ole Lynggaard's AR try-on — into Chinese operations manuals for the Zhuyun team. I deployed a 6-agent workforce: an own-brand positioning agent that mined the 15-year OEM archive (designs made for 30+ European brands) to extract Zhuyun's unique “Pearl River Rhyme” design DNA; a Xiaohongshu quiet-luxury content agent posting 4 brand-story posts a week; a Douyin livestream sales agent assisting hosts in real time (product info, pricing, client Q&A prepped 24/7); a WeChat private-domain VIP agent managing the high-net-worth client base; a PIPL compliance data-localization agent ensuring data never leaves China; and an OEM client conversion agent identifying which European OEM clients could become B2B clients of the Zhuyun own brand.",
    solutionZh:
    ...
    results: [
      { label: "Own-brand revenue Year 1", labelZh: "自主品牌首年营收", before: "0", after: "1.8 亿人民币（超目标 80%）", beforeEn: "0", afterEn: "RMB 180M (80% above target)" },
      { label: "Xiaohongshu followers", labelZh: "小红书粉丝", before: "0", after: "87,000（8 个月）", beforeEn: "0", afterEn: "87,000 (8 months)" },
      { label: "Douyin livestream GMV / month", labelZh: "抖音直播月均 GMV", before: "0", after: "4,200 万人民币", beforeEn: "0", afterEn: "RMB 42M" },
      { label: "VIP customer repeat rate", labelZh: "VIP 客户复购率", before: "0", after: "42%（行业均值 18%）", beforeEn: "0", afterEn: "42% (industry average 18%)" },
    ],
    timeline: [
      { week: "Week 1-3", title: "OEM asset audit + brand DNA extraction", titleZh: "OEM 资产审计 + 品牌 DNA 提取", detail: "15-year OEM archive analyzed; Pearl River Rhyme design DNA locked; signed off by the CEO + design director.", detailZh: "分析 15 年 OEM 档案；锁定珠江韵设计 DNA；CEO + 设计总监签字确认。" },
      { week: "Week 4-6", title: "Xiaohongshu + Douyin content engine", titleZh: "小红书 + 抖音内容引擎", detail: "First 32 Xiaohongshu posts published; Douyin content calendar locked.", detailZh: "发布首批 32 条小红书帖子；锁定抖音内容日历。" },
      { week: "Week 7-10", title: "WeChat private domain + PIPL compliance", titleZh: "微信私域 + PIPL 合规", detail: "Aliyun Shanghai deployment live; PIPL compliance audit passed; WeChat private domain launched.", detailZh: "阿里云上海区部署上线；PIPL 合规审计通过；微信私域启动。" },
      { week: "Week 11-14", title: "Douyin livestream agent + OEM client conversion", titleZh: "抖音直播代理 + OEM 客户转移", detail: "Livestream assistant agent live; first 3 European OEM clients approached for own-brand B2B.", detailZh: "直播辅助代理上线；首批 3 家欧洲 OEM 客户接洽自主品牌 B2B 业务。" },
    ],
    agents: ["Own-Brand Positioning Agent", "Xiaohongshu Quiet-Luxury Content Agent", "Douyin Livestream Sales Agent", "WeChat Private-Domain VIP Agent", "PIPL Data-Localization Agent", "OEM Client Conversion Agent"],
    ...
    quoteAuthorEn: "Chen Zhiyuan, CEO, Zhuyun Jewelry",
    designDna: "OEM heritage (15 years manufacturing for 30+ European luxury brands) + Pearl River Rhyme (珠江韵) own-brand DNA — Cantonese craft meets quiet luxury.",
    designDnaZh: "OEM 传承（15 年为 30+ 欧洲奢侈品牌代工）+ 珠江韵自主品牌 DNA — 粤式工艺 × 静奢美学。",
    keyCollections: ["珠江韵系列（首发）", "南沙日出 18k 金项链", "番禺工匠签名戒", "粤港澳大湾区限量"],
    keyCollectionsEn: ["The Pearl River Rhyme collection (debut)", "Nansha Sunrise 18k gold necklace", "The Panyu Artisan signature ring", "Greater Bay Area limited edition"],
    materials: ["18k 金", "925 银", "钻石（实验室 + 天然）", "珐琅", "珍珠"],
    materialsEn: ["18k gold", "925 silver", "diamonds (lab-grown + natural)", "enamel", "pearls"],
    myRole: "我在珠韵的角色是方法转移：把丹麦验证过的部署框架翻成中文操作手册，培训他们的 5 人数字小组，并陪跑前 14 周。代理上线后的日常运营 100% 由珠韵团队自己开——我至今远程，一周看一次数据。",
    myRoleEn: "My role at Zhuyun was method transfer: turning the proven Danish deployment framework into Chinese operations manuals, training their 5-person digital team, and shadowing the first 14 weeks. Day-to-day operation after launch is 100% Zhuyun's own team — I stay remote and check the numbers once a week.",
    ...
    mechanicsEn: "The Douyin livestream assist runs as real-time earpiece prompting: the host on camera, an operator reading AI-prepared product specs and answers alongside. The first thing that truly clicked — in week 6 — was the private-domain SOP: add follower, tag, first touch within 48 hours; before that, followers were added and left sitting. Six of the first 32 Xiaohongshu posts were drafted by me in Chinese (my Chinese writing makes frequent mistakes; the design director fixed them line by line) — all posts are now written by the Zhuyun team themselves. The final call on the “Pearl River Rhyme” design DNA was made by Mr. Chen and the design director — the model only supplied options.",
    ...
    frictionEn: { problem: "The PIPL compliance audit on Aliyun's Shanghai region ran 11 days behind schedule — the third-party security-level assessment (MLPS) schedule collided, and the WeChat private-domain launch had to be postponed.", fix: "We split the private domain into two steps: run follower acquisition and tagging on WeCom first (outside the MLPS scope), then migrate data to the main database in one move once the audit passed. Mr. Chen later said those 11 days forced out a cleaner architecture." },
  },
];
